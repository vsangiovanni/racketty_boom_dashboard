...
const { hasTransactionsCompanyColumn, buildLedgerWhereClause } = require('./lib/ledgerFilters');
const { createSchemaApi } = require('./schema/ensureSchema');
...
// QUOTE_NOTIFY_TO en backend/.env (separar con coma o punto y coma). Si no existe o está vacío:
const QUOTE_NOTIFY_TO_FALLBACK = process.env.QUOTE_NOTIFY_TO || 'victorsangiovanni@gmail.com,gregorybarneswatson@gmail.com';
...
const { ensureSchema, propagateVendorRenameToLedger } = createSchemaApi(pool);
if (
...
async function sendQuoteRequestNotificationEmail(detail) {
  const teamRecipients = parseQuoteNotifyRecipients(
    String(process.env.QUOTE_NOTIFY_TO || '').trim() || QUOTE_NOTIFY_TO_FALLBACK
  );
